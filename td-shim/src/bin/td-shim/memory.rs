...
impl<'a> Memory<'a> {
    pub fn new(
        ...
        let payload_type = payload_info
            ...
            .unwrap_or(TdKernelInfoHobType::ExecutablePayload);
        ...
        let layout_config = match payload_type {
            TdKernelInfoHobType::ExecutablePayload => runtime::exec::MEMORY_LAYOUT_CONFIG,
            TdKernelInfoHobType::BzImage | TdKernelInfoHobType::RawVmLinux => {
                runtime::linux::MEMORY_LAYOUT_CONFIG
            }
            TdKernelInfoHobType::UnknownImage => return None,
        };
        let layout = RuntimeLayout::new(tolm as usize, layout_config)?;
    ...
    pub fn create_e820(&self) -> E820Table {
        ...
        for r in self.layout.regions() {
            if r.r#type != E820Type::Memory {
                table.convert_range(r.r#type, r.base_address as u64, r.size as u64);
            }
        }
    ...
    #[cfg(all(feature = "tdx", feature = "linux-payload", feature = "lazy-accept"))]
    ...
    pub fn build_unaccepted_memory_bitmap(&self) -> u64 {
        ...
        let bitmap = self.get_dynamic_mem_slice_mut(memslice::SliceType::LazyAcceptBitmap);
        ...
        self.layout.runtime_lazy_accept_bitmap_base
    }
