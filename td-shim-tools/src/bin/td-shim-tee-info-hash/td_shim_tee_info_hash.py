...
class TdxMetadataSection:
    ...
    STRUCT_FORMAT = "<IIQQII"  # data_offset, raw_data_size, memory_address, memory_data_size, type, attributes
    SIZE = 32
...
def build_mrtd(image_file: BinaryIO, image_size: int, add_all: bool) -> bytes:
    ...
    if compare_guid(footer_guid_buf, OVMF_TABLE_FOOTER_GUID):
        ...
        while count < table_len:
            ...
            if compare_guid(guid_buf, OVMF_TABLE_TDX_METADATA_GUID):
                ...
                metadata_offset_value = struct.unpack("<I", image_file.read(4))[0]
    ...
    for _ in range(descriptor.number_of_section_entry):
        sec_data = metadata_buf[desc_offset:desc_offset + TdxMetadataSection.SIZE]
        ...
        if (sec.type != TDX_METADATA_SECTION_TYPE_TD_INFO and
            ...
            raise ValueError("Memory data size must exceed or equal the raw data size!")
        ...
        if add_all:
            # Process all pages: first all PAGE.ADD, then all MR.EXTEND
...
def fill_buffer3_128_with_mr_extend(
    ...
    if len(data1) < MRTD_EXTENSION_BUFFER_SIZE:
        raise IOError(f"Failed to read enough data for buffer[1]: got {len(data1)}, expected {MRTD_EXTENSION_BUFFER_SIZE}")
    if len(data2) < MRTD_EXTENSION_BUFFER_SIZE:
        raise IOError(f"Failed to read enough data for buffer[2]: got {len(data2)}, expected {MRTD_EXTENSION_BUFFER_SIZE}")
...
def main():
    ...
    parser.add_argument(
        ...
        const=True,
        default=True,
        type=lambda x: x.lower() in ('true', '1', 'yes', 'on') if x else True,
        help="Boolean flag to specify the behavior of Qemu. "
             "If true, use the first behavior (all MR.PAGE.ADD then all MR.EXTEND), "
             "otherwise use the second behavior (MR.PAGE.ADD and MR.EXTEND separate pages one by one). "
             "[default: True]"
    )
    ...
    with open(args.image, 'rb') as image_file:
        ...
        mrtd = build_mrtd(image_file, image_size, args.add_all)
