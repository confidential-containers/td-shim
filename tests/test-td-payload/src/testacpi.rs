...
impl Default for TdAcpiData {
    fn default() -> TdAcpiData {
        TdAcpiData {name: String::default(), signature: [0,0,0,0], valid: 0, exist: 0}
    }
}
...
impl TdAcpi {
    ...
    fn parse_hob(&self, hob: usize) -> Vec<TdAcpiData> {
        let mut tables : Vec<TdAcpiData> = Vec::new();

        // Parse Hob to populate td_acpi_list
        let hob_buffer = unsafe {
            memslice::get_dynamic_mem_slice_mut(memslice::SliceType::PayloadHob, hob)
        };
    ...
    fn verify_tables(&self, acpi_tables: Vec<TdAcpiData>) -> TestResult {
        ...
        let mut cnt: usize = 0;
        while cnt < self.expected.num as usize {
            let expected_signature = self.expected.tables[cnt].signature;

            let index = acpi_tables.iter().position(|r| r.signature == expected_signature);
            // if the ACPI is not found
            if index.is_none() {
                log::info!("ACPI {} is not found.", String::from_utf8_lossy(&expected_signature));
                return TestResult::Fail;
            }
            ...
            if acpi_tables[idx].valid == 0 {
                ...
                log::info!("ACPI {} is not valid.", String::from_utf8_lossy(&expected_signature));
                return TestResult::Fail;
            }
...
impl TestCase for TdAcpi {
    ...
    fn run(&mut self) {
        ...
        let acpi_tables = self.parse_hob(self.hob);

        self.result = self.verify_tables(acpi_tables);
    }
